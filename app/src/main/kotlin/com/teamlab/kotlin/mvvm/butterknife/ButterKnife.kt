...
private val fragmentViewCache: WeakHashMap<View, MutableMap<Int, View>> = WeakHashMap()
private val Fragment.viewCache: MutableMap<Int, View>
    get() = view?.let {
...
private class FragmentViewProperty<V : View>(private val id: Int) : ReadOnlyProperty<Fragment, V> {
    override fun getValue(thisRef: Fragment, property: KProperty<*>): V {
        ...
        return thisRef.viewCache.getOrPut(id, {
            thisRef.view.findViewById(id)
                    ?: throw NullPointerException("View ID $id for '${property.name}' not found.")
        }) as V
    }
}

private val dialogFragmentDialogCache: WeakHashMap<Dialog, MutableMap<Int, View>> = WeakHashMap()
private val DialogFragment.dialogCache: MutableMap<Int, View>
    get() = dialog?.let {
...
private class DialogFragmentViewProperty<V : View>(private val id: Int) : ReadOnlyProperty<DialogFragment, V> {
    override fun getValue(thisRef: DialogFragment, property: KProperty<*>): V {
        @Suppress("UNCHECKED_CAST")
        return thisRef.dialogCache.getOrPut(id, {
            thisRef.dialog.findViewById(id)
                    ?: throw NullPointerException("View ID $id for '${property.name}' not found.")
        }) as V
    }
}
